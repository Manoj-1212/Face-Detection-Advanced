    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                tim = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
                ...
                myobj = gTTS(text=str(bb), lang=language, slow=False)
                if bb in present:
                    pass
                else:
                    myobj.save(str(bb)+str(tim)+"-welcome.mp3")
                    mixer.init()
                    mixer.music.load(str(bb)+str(tim)+"-welcome.mp3")
                    mixer.music.play()
                    ...
                    mixer.music.unload()
                    present.append(bb)
                    os.remove(str(bb)+str(tim)+"-welcome.mp3")
            else:
                Id = 'Unknown'
                bb = str(Id)
